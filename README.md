# 👋 Hi, I'm Mahmudul Hasan Badhan

🎓 ** Research Assistant | ML, NLP & Speech Processing**

I am a Computer Science graduate from **Shahjalal University of Science and Technology (SUST)** with a strong research focus on **Machine Learning, Natural Language Processing, and Speech Processing**, particularly for **low-resource languages** like Bangla.

I am currently working as a **Research Assistant**, contributing to multiple deep learning–based research projects. My undergraduate thesis explored **zero-shot Bengali voice cloning** using neural network–based speaker encoders, speaker converters, and **VITS-based TTS** under limited-data settings.

---

## 🔬 Research Interests

* Natural Language Processing (NLP)
* Speech Processing & Text-to-Speech (TTS)
* Zero-shot & Low-resource Learning
* Deep Learning for Indigenous & Regional Languages

---

## 🧠 Technical Skills

* **Machine Learning & Deep Learning**
* **PyTorch**, Hugging Face, Scikit-learn
* NLP, Speech Processing, VITS TTS
* Computer Vision
* Python, JavaScript, React.js
* MongoDB, Firebase

---

## 📄 Publications (IEEE)

**Accepted – ICCIT 2025**

* A Comparative Study of Deep Learning Models for Bias Detection in Bangla Job Advertisements
* Sentence-Level Indigenous Language Identification in Bangladesh
* Exploring Regional Variation in Bangla: Automating Dialect Classification Using Deep Learning
* Deep Learning for Automated Poem Meter Classification in Bangla
* Ethnic Language Classification in Bangladesh Using Machine Learning

---

## 💼 Experience

**Research Assistant**
Working on machine learning and deep learning–based research projects in NLP and speech processing.

**AI Engineer (Remote) – Debugger Lab**
Developed and deployed AI models and data pipelines for production use.

**Python Instructor – ICGS**
Taught Python fundamentals and real-world programming applications.

---

## 🌐 Links

* 🔗 **Kaggle**: [https://www.kaggle.com/mahmudulhasanbadhan](https://www.kaggle.com/mahmudulhasanbadhan)
* 🔗 **Portfolio**: [https://mahmudulhasanbadhan.netlify.app/)
* 📧 **Email**: [mahmudulhasanw3b@gmail.com](mailto:mahmudulhasanw3b@gmail.com)

---
